...
macro_rules! __append_html {
    ...
    ($tmpl:ident, $tag:ident($($attrs:tt)+) $tag2:ident $first:tt $($next:tt)* ) => {
        __append_html!($tmpl, $tag($($attrs)*) { $tag2 $first $($next)* });
    };
    ($tmpl:ident, $tag:ident $tag2:ident $first:tt $($next:tt)* ) => {
        __append_html!($tmpl, $tag { $tag2 $first $($next)* });
    };
    ($tmpl:ident, $tag:ident($($attrs:tt)+) $tag2:ident) => {
        __append_html!($tmpl, $tag($($attrs)*) { $tag2 });
    };
    ($tmpl:ident, $tag:ident $tag2:ident) => {
        __append_html!($tmpl, $tag { $tag2 });
    };
